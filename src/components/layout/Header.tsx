        ...
        <header className="fixed top-0 left-0 w-full z-50 bg-white py-4 border-b border-slate-100">
            ...
            {isMenuOpen && (
                <div className="lg:hidden absolute top-full left-0 w-full bg-white border-b border-slate-100 p-6 flex flex-col gap-4 animate-in slide-in-from-top duration-300">
                    <Link href="/" prefetch={true} className="font-bold text-primary text-lg" onClick={() => setIsMenuOpen(false)}>Home</Link>
                    <Link href="/free-brand-logos" prefetch={true} className="font-semibold text-slate-700 text-lg" onClick={() => setIsMenuOpen(false)}>Free Brand Logos</Link>
                    <Link href="/royalty-free-music" prefetch={true} className="font-semibold text-slate-700 text-lg" onClick={() => setIsMenuOpen(false)}>Royalty Free Music</Link>
                    <Link href="/blog" prefetch={true} className="font-semibold text-slate-700 text-lg" onClick={() => setIsMenuOpen(false)}>Blog</Link>
                    <Link href="/faq" prefetch={true} className="font-semibold text-slate-700 text-lg" onClick={() => setIsMenuOpen(false)}>FAQ</Link>
                </div>
            )}
        </header>
    );
