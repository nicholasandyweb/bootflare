...
export async function generateMetadata({ params }: { params: Promise<{ page: string }> }): Promise<Metadata> {
    ...
    if (parseInt(page) > 1 && metadata.title) {
        metadata.title = `${metadata.title} - Page ${page}`;
    }
...
export default async function RoyaltyFreeMusicPagination({ params }: { params: Promise<{ page: string }> }) {
    ...
    try {
        ...
        console.warn('GraphQL failed for RoyaltyFreeMusicPagination, falling back to REST:', error);
        try {
            ...
            <div className="bg-[#FBF3FF] py-16 px-6 pt-32 pb-16">
                <div className="container max-w-4xl mx-auto text-center">
                    <h1 className="text-4xl md:text-[2.75rem] font-bold mb-6 text-slate-800">
                        {wpData?.page?.title ? <span dangerouslySetInnerHTML={{ __html: wpData.page.title }} /> : <>Royalty Free Music Download</>}
                        {page > 1 && <span className="text-primary ml-4 opacity-50">#Page {page}</span>}
                    </h1>
                    ...
                    <div className="flex items-center justify-center gap-1.5 text-[15px] mb-8 text-slate-600">
                        ...
                        <Link href="/royalty-free-music" className="text-[#8b5cf6] hover:underline font-medium">Royalty Free Music</Link>
                        {page > 1 && (
                            <>
                                <span className="text-slate-400 text-xs mt-0.5">»</span>
