...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="bg-white min-h-screen pb-20" suppressHydrationWarning>
      {/* Article Progress Bar UI placeholder or just spacer */}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-8">
            {post.categories.nodes.map(cat => (
              <span key={cat.slug} className="text-[10px] font-bold text-primary uppercase tracking-widest px-3 py-1 bg-primary/10 rounded-full">
                {cat.name}
              </span>
            ))}
